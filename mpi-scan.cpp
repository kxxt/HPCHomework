...
int main(int argc, char **argv) {
    ...
    MPI_Scan(&buf, &result, 1, MPI_BYTE, MPI_BXOR, MPI_COMM_WORLD);
    printf("Rank %.2d: Got result " BYTE_TO_BINARY_PATTERN "\n", rank, BYTE_TO_BINARY(result));
    fflush(stdout);
    MPI_Barrier(MPI_COMM_WORLD);
    usleep(rank * 1000);
    printf("Rank %d: Showing the mechanism.\n", rank);
    for (int i = 0; i < rank; i++) {
        printf("\t" BYTE_TO_BINARY_PATTERN "\n", BYTE_TO_BINARY((unsigned char) i));
    }
    printf("XOR\t" BYTE_TO_BINARY_PATTERN "\n", BYTE_TO_BINARY((unsigned char) rank));
    printf("-----------------\n");
